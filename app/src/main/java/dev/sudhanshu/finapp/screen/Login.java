...
import com.app.makeassure.databinding.LoginLayoutBinding;
...
import dev.sudhanshu.finapp.PreSaveUser;
...
public class Login extends AppCompatActivity {
    ...
    private void validateInput() {
        ...
        }else if (binding.email.getText().toString().trim().length() < 10) {
            binding.email.requestFocus();
            binding.email.setError("Username must be 10 characters");
        }else if (binding.password.getText().toString().trim().isEmpty()) {
            binding.password.requestFocus();
            binding.password.setError("Please enter your password");
        } else if (!isValidPassword(binding.password.getText().toString().trim())) {
            binding.password.requestFocus();
            binding.password.setError("Password must be at least 7 characters long with one uppercase alphabet, one numeric digit, and one special character.");
        } else {
    ...
    private void moveToHome() {
        ...
        pairs.add(new Pair<>(binding.appLogo, "titleTransition"));
        ActivityOptions activityOptions = ActivityOptions.makeSceneTransitionAnimation(this, pairs.toArray(new Pair[0]));
